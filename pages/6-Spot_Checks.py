...
st.session_state.setdefault("spot_lock_gid", None)            # FIX: lock current group during refresh
...
# FIX: if a second opinion is in-flight, lock to that group_id even if list order changed
if st.session_state.spot_lock_gid is not None:
    _locked_gid = st.session_state.spot_lock_gid
    locked_rows = candidates[candidates["Group ID"] == _locked_gid]
    if not locked_rows.empty:
        ...
        # fallback if it somehow dropped from pool
...
with col2:
    ...
    if (not ai_label) and (not st.session_state.spot_ai_loading):
        ...
        st.session_state.spot_lock_gid = current_group_id     # FIX: lock while fetching
        st.rerun()
    ...
    if st.session_state.spot_ai_loading and st.session_state.spot_ai_refresh_requested:
        ...
        st.session_state.spot_lock_gid = None                  # FIX: release lock
        st.rerun()
    ...
    with sec_col:
        if st.button("↻ Second opinion", disabled=st.session_state.spot_ai_loading, use_container_width=True):
            # FIX: do NOT clear AI columns; just set override + lock and rerun
            st.session_state.spot_ai_loading = True
            st.session_state.spot_ai_refresh_requested = True
    ...
    with prev_col:
        if st.button("◄ Back", disabled=(idx <= 0), use_container_width=True):
            ...
            st.session_state.spot_lock_gid = None   # ensure we’re not locking the previous row
            st.rerun()
    with next_col:
