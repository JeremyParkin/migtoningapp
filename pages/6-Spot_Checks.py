...
with col2:
    ...
    # with st.expander("Reset these counters", expanded=False):
    #     if st.button("Reset (counters only)"):
    #         st.session_state.spot_checked_groups = set()
    #         st.session_state.accepted_initial = set()
    #         st.rerun()
...
# st.session_state.setdefault("spot_checked_groups", set()) # set of group_ids
# st.session_state.setdefault("accepted_initial", set())    # subset of above
# st.session_state.setdefault("spot_idx", 0)                # cursor in candidate list
#
# # ==================== Config + Columns ====================
# _raw_st = st.session_state.get("sentiment_type", "3-way")
# _s = str(_raw_st).strip().lower()
...
#
# def set_assigned_sentiment(group_id, label):
#     st.session_state.unique_stories.loc[st.session_state.unique_stories["Group ID"] == group_id, "Assigned Sentiment"] = label
#     st.session_state.df_traditional.loc[st.session_state.df_traditional["Group ID"] == group_id, "Assigned Sentiment"] = label
#
# def get_group_count(gid: int) -> int:
#     try:
...
#
# # ==================== Candidate Scoring ====================
# def compute_candidates(conf_thresh: int):
#     us = st.session_state.unique_stories.copy()
#     dt = st.session_state.df_traditional
#
#     # groups with a human label in either DF
#     assigned_us = set(us.loc[us["Assigned Sentiment"].notna(), "Group ID"].unique())
#     assigned_dt = set(dt.loc[dt["Assigned Sentiment"].notna(), "Group ID"].unique())
#     assigned_any = assigned_us | assigned_dt
#
#     # start from unique_stories, exclude assigned groups, and require AI present
#     pool = us[(~us["Group ID"].isin(assigned_any)) & (us["AI Sentiment"].notna())].copy()
#     if pool.empty:
#         return pool
#
...
#         pool["_IMP"] = pd.to_numeric(pool[imp_col], errors="coerce").fillna(0)
#         max_imp = pool["_IMP"].max()
#         pool["IMP_NORM"] = (pool["_IMP"] / max_imp) if max_imp > 0 else 0
#     else:
#         pool["IMP_NORM"] = 0.0
#
#     pool["SCORE"] = (
#         W_GROUP*pool["GC_NORM"] + W_NEG*pool["NEG_SCORE"] + W_IMP*pool["IMP_NORM"] + W_LOWCF*pool["LOWCONF"]
#     )
#     return pool.sort_values(["SCORE","GROUP_CT","IMP_NORM"], ascending=[False,False,False]).reset_index(drop=True)
#
# #
# # def compute_candidates(conf_thresh: int):
...
# #         pool["IMP_NORM"] = 0.0
# #
# #     pool["SCORE"] = W_GROUP*pool["GC_NORM"] + W_NEG*pool["NEG_SCORE"] + W_IMP*pool["IMP_NORM"] + W_LOWCF*pool["LOWCONF"]
# #     return pool.sort_values(["SCORE","GROUP_CT","IMP_NORM"], ascending=[False,False,False]).reset_index(drop=True)
#
# # ==================== Build Candidate List ====================
# candidates = compute_candidates(CONF_THRESH)
...
# if len(candidates) == 0:
#     st.success("All set — no remaining stories need spot checks.")
#     # Bottom-of-page metrics (still shown)
#     checked = len(st.session_state.spot_checked_groups)
#     accepted = len(st.session_state.accepted_initial)
#     rate = (accepted / checked) if checked else 0.0
#     _, _ = st.columns(2)  # maintain spacing like the page layout
#     m1, m2 = st.columns(2)
#     with m1: st.metric("Spot-checked", checked)
#     with m2: st.metric("Acceptance rate", f"{rate:.0%}")
#     st.stop()
...
#     accept_help = None if not accept_disabled else "AI label not ready yet."
#
#     def _finalize_and_advance(final_label: str):
#         set_assigned_sentiment(current_group_id, final_label)
#         st.session_state.spot_checked_groups.add(int(current_group_id))
#         init_map = st.session_state.initial_ai_label
#         if (int(current_group_id) in init_map and
#             final_label.strip().upper() == str(init_map[int(current_group_id)]).strip().upper()):
#             st.session_state.accepted_initial.add(int(current_group_id))
#         # Recompute candidates and advance
#         new_candidates = compute_candidates(CONF_THRESH)
#         if new_candidates.empty:
#             st.rerun()
#         else:
#             st.session_state.spot_idx = min(idx, len(new_candidates)-1)
#             st.rerun()
#
#     with acc_col:
#         if st.button("✅ Accept opinion", disabled=accept_disabled, help=accept_help, use_container_width=True):
#             if ai_label:
#                 _finalize_and_advance(str(ai_label).strip())
#
#     with sec_col:
#         if st.button("↻ Second opinion", disabled=st.session_state.spot_ai_loading, use_container_width=True):
#             st.session_state.spot_ai_loading = True
#             st.session_state.spot_ai_refresh_requested = True
#             st.session_state.spot_ai_model_override = SECOND_OPINION_MODEL  # GPT-5 for this run
#             for df_name in ["unique_stories","df_traditional"]:
#                 df = st.session_state.get(df_name)
#                 if isinstance(df, pd.DataFrame):
#                     mask = df["Group ID"] == current_group_id
...
#
#     if clicked_override:
#         _finalize_and_advance(clicked_override)
#
#     st.divider()
#
#     # Navigation (metrics will be shown beneath to match Toning page)
#     prev_col, next_col = st.columns(2)
#     with prev_col:
#         if st.button("◄ Back", disabled=(idx == 0), use_container_width=True):
#             st.session_state.spot_idx = max(0, idx - 1)
...
#             st.rerun()
#
#     # ---- Metrics below nav (Spot-checked count + Acceptance rate %) ----
#     # checked = len(st.session_state.spot_checked_groups)
#     # accepted = len(st.session_state.accepted_initial)
#     # rate = (accepted / checked) if checked else 0.0
#     # m1, m2 = st.columns(2)
#     # with m1: st.metric("Spot-checked", checked)
#     # with m2: st.metric("Acceptance rate", f"{rate:.0%}") # checked = len(st.session_state.spot_checked_groups)
#     # accepted = len(st.session_state.accepted_initial)
#     # rate = (accepted / checked) if checked else 0.0
#     # m1, m2 = st.columns(2)
#     # with m1: st.metric("Spot-checked", checked)
...
#
#     remaining_cnt = len(candidates)  # unique stories still eligible right now
#     priority_score = float(row.get("SCORE", 0.0))  # computed in compute_candidates
#
#     m1, m2 = st.columns(2)
#     with m1:
#         st.metric("Spot-checked", checked)
#     with m2:
#         st.metric("Acceptance rate", f"{rate:.0%}")
#     m3, m4 = st.columns(2)
#     with m3:
#         st.metric("Remaining", remaining_cnt)
#     with m4:
#         st.metric("Priority score", f"{priority_score:.0%}")
#
#     # with st.expander("Reset these counters", expanded=False):
#     #     if st.button("Reset (counters only)"):
#     #         st.session_state.spot_checked_groups = set()
...
#
# # Footer hint
# st.caption("This list updates automatically as you accept/override labels. Items drop out once they have an **Assigned Sentiment**. Acceptance rate is vs the original AI label.")
