...
with sidebar:
    if st.button("Translate"):
        try:
            ...
            # Update in-memory filtered row for immediate display
...
with col2:
    ...
    # If a refresh/first-load is requested, do the call here (keeps buttons disabled while working)
    if st.session_state.ai_loading and st.session_state.ai_refresh_requested:
        ...
        # flip flags and rerun to render with enabled buttons
        st.session_state.ai_loading = False
        st.session_state.ai_refresh_requested = False
        st.rerun()

    # Read again after any possible refresh above
    row_now = st.session_state.filtered_stories.iloc[counter]
    ai_label = row_now.get("AI Sentiment")
    ai_rsn   = row_now.get("AI Sentiment Rationale")
    ...
    # --- Accept + Second opinion side-by-side (both disabled while loading; Accept also disabled if no AI yet) ---
    ...
    with sec_col:
        # Second opinion triggers a refresh; disabled while loading
        if st.button("↻ Second opinion", disabled=st.session_state.ai_loading, help=None if not st.session_state.ai_loading else "AI is working…"):
            st.session_state.ai_loading = True
            st.session_state.ai_refresh_requested = True
