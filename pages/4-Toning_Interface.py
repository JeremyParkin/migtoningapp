...
# Model resolver (avoid non-existent models)
def resolve_model_choice(choice: str) -> str:
    # Prefer gpt-5-mini by default and allow explicit pass-through
    if not choice:
        return "gpt-5-mini"
...
with col2:
    st.caption(f"Active sentiment label set: **{sentiment_type}**")

    # Translate button
    tcol1, _ = st.columns(2)
    with tcol1:
        if st.button("Translate"):
            st.session_state.filtered_stories.at[counter, "Translated Headline"] = translate(head_raw)
            st.session_state.filtered_stories.at[counter, "Translated Body"] = translate(body_raw)
            st.rerun()
    ...
    st.markdown("**AI Sentiment Opinion**")
    sentiment_placeholder = st.empty()
    story_prompt = build_story_prompt(head_raw, body_raw)

    # Button to force a fresh opinion
    regen = st.button("↻ Regenerate AI opinion", key=f"regen_{current_group_id}")

    # If regenerate requested, clear AI fields for this group in all DFs
    if regen:
    ...
    # Read latest (after optional clear)
    # Ensure we look at the freshest row (filtered_stories may have been updated)
    ...
    if ai_label and not regen:
        # Show cached
        lines = [f"**Label:** {ai_label}"]
        if ai_conf is not None: lines.append(f"**Confidence:** {ai_conf}%")
        if ai_rsn: lines.append(f"**Why:** {ai_rsn}")
        ...
        if ai_result:
            ...
            # Pretty print
            out = [f"**Label:** {label or '(none)'}"]
            if conf is not None: out.append(f"**Confidence:** {conf}%")
            if why: out.append(f"**Why:** {why}")
            sentiment_placeholder.write("\n\n".join(out))
...
#     return "gpt-4o-mini"
#
# model_id = resolve_model_choice(st.session_state.get("model_choice", "gpt-4o-mini"))
#
# # Dataframes setup
# if "Assigned Sentiment" not in st.session_state.df_traditional.columns:
#     st.session_state.df_traditional["Assigned Sentiment"] = pd.NA
# for df_name in ["unique_stories", "df_traditional"]:
#     df = st.session_state.get(df_name, pd.DataFrame())
#     # if isinstance(df, pd.DataFrame) and "Sentiment Opinion" not in df.columns:
#     #     df["Sentiment Opinion"] = None
#     #     st.session_state[df_name] = df
#
# # Story list and counter
# st.session_state.filtered_stories = st.session_state.unique_stories.copy()
# st.session_state.setdefault("counter", 0)
# counter = st.session_state.counter
#
# # Keywords (compiled on previous page)
# keywords = st.session_state.get("highlight_keyword", [])
# if not isinstance(keywords, list):
#     keywords = [str(keywords)] if keywords else []
# keywords = [k for k in keywords if isinstance(k, str) and k.strip()]
...
#     for k in kw:
#         k_esc = re.escape(k)
#         # Use word boundaries for single terms; allow spaces within phrases
#         escaped.append(rf"\b{k_esc}\b" if " " not in k else k_esc)
#     pattern = r"(?:%s)" % "|".join(escaped)
#     def repl(m):
...
#
# def translate_concurrently(chunks):
#     """Concurrent translate preserving order; avoids tricky dict-comprehension syntax."""
#     translator = GoogleTranslator(source="auto", target="en")
#     results = [None] * len(chunks)
#     with ThreadPoolExecutor(max_workers=30) as ex:
#         futures = [(i, ex.submit(translator.translate, chunk)) for i, chunk in enumerate(chunks)]
#         for i, fut in futures:
#             try:
...
#     return " ".join(translate_concurrently(chunks))
#
# def build_story_prompt(headline: str, snippet: str) -> str:
#     parts = []
#     if pre_prompt:
...
# def call_ai_sentiment(story_prompt: str):
#     """
#     Try function-calling first (legacy 'functions' path).
#     Fallback to plain text and do a light parse.
...
#             len(st.session_state.filtered_stories) - 1, st.session_state.counter + 1
#         )
#         st.rerun()
#
#     # Navigation
...
#         if st.button(
#             "Next",
#             disabled=(st.session_state.counter == len(st.session_state.filtered_stories) - 1),
#         ):
#             st.session_state.counter = min(
#                 len(st.session_state.filtered_stories) - 1, st.session_state.counter + 1
#             )
...
#     regen = st.button("↻ Regenerate AI opinion", key=f"regen_{current_group_id}")
#
#     # If regenerate requested, clear cached opinion for this group in all DFs
#     if regen:
#         for df_name in ["filtered_stories", "unique_stories", "df_traditional"]:
#             df = st.session_state.get(df_name)
#             # if isinstance(df, pd.DataFrame) and "Sentiment Opinion" in df.columns:
#             #     df.loc[df["Group ID"] == current_group_id, "Sentiment Opinion"] = None
#
#     existing_opinion = row.get("Sentiment Opinion")
#     if pd.notna(existing_opinion) and existing_opinion:
#         sentiment_placeholder.write(existing_opinion)
#     else:
#         with st.spinner("Generating AI opinion..."):
#             ai_result = call_ai_sentiment(story_prompt)
#         if ai_result:
...
#                 op_lines.append(f"{ai_result['sentiment']}")
#             # if ai_result.get("confidence") is not None:
#             #     op_lines.append(f"**Confidence:** {ai_result['confidence']}%")
#             if ai_result.get("explanation"):
#                 op_lines.append(f"{ai_result['explanation']}")
#             if not op_lines:
#                 op_lines = ["(No structured output returned.)"]
#             opinion_md = "\n\n".join(op_lines)
#             sentiment_placeholder.write(opinion_md)
#
#             # Persist to all DFs for this Group ID
#             st.session_state.filtered_stories.at[counter, "Sentiment Opinion"] = opinion_md
#             st.session_state.unique_stories.loc[
#                 st.session_state.unique_stories["Group ID"] == current_group_id, "Sentiment Opinion"
#             ] = opinion_md
#             st.session_state.df_traditional.loc[
#                 st.session_state.df_traditional["Group ID"] == current_group_id, "Sentiment Opinion"
#             ] = opinion_md
#
