...
# st.session_state.setdefault("ai_model_override", None)  # one-shot override holder
#
# # Summarizer model for building Option-B examples (cheap & fast)
# EX_SUMMARIZER_MODEL = "gpt-5-mini"
#
# # --- Normalise sentiment type (3-way vs 5-way) ---
# _raw_st = st.session_state.get("sentiment_type", "3-way")
# _s = str(_raw_st).strip().lower()
...
# st.session_state.setdefault("ai_refresh_requested", False)
#
# # --- Calibration (Option B) state ---
# st.session_state.setdefault("calibration_pool", [])          # raw seeds (human-toned rows)
# st.session_state.setdefault("calibration_seen_gids", set())  # to avoid duplicate seeds per group
# st.session_state.setdefault("calibration_examples", [])      # distilled exemplars ready for bulk
# st.session_state.setdefault("calibration_last_status", "")   # small status string
#
# # ===================== Utils =====================
# def escape_markdown(text: str) -> str:
#     """Escape Markdown special chars outside of URLs for safe st.markdown rendering."""
...
#         return 1
#
# # -------- Calibration helpers (Option B: distilled exemplars) --------
# EXAMPLE_CAP = 20  # how many exemplars you want to keep handy for bulk
#
...
#
# def _add_calibration_seed(group_id: int, human_label: str, head_raw: str, body_raw: str):
#     """Add a unique seed for this group to the calibration pool (prefer translated text)."""
#     seen = st.session_state.calibration_seen_gids
#     if group_id in seen:
#         return
#     # Prefer translated text if present
#     row_all = st.session_state.df_traditional[st.session_state.df_traditional["Group ID"] == group_id].iloc[0]
#     th = row_all.get("Translated Headline")
#     tb = row_all.get("Translated Body")
#     story_head = th if isinstance(th, str) and th.strip() else head_raw
#     story_body = tb if isinstance(tb, str) and tb.strip() else body_raw
#
#     seed = {
#         "group_id": int(group_id),
#         "label": str(human_label).strip().upper(),
#         "headline": str(story_head or "")[:400],
#         "snippet": str(story_body or "")[:2500],
#     }
#     st.session_state.calibration_pool.append(seed)
#     seen.add(group_id)
#
# def _summarize_seed_to_exemplar(seed: dict, named_entity: str, label_set: list[str]) -> dict | None:
#     """Turn one seed into a compact exemplar dict. Returns None on failure."""
#     text = f"{seed.get('headline', '')}\n\n{seed.get('snippet', '')}"
#     nes = named_entity
#     label = seed.get("label") or "NEUTRAL"
#     labels_csv = ", ".join(label_set)
#
#     prompt = f"""
# You are preparing distilled training examples for sentiment toning toward {nes}.
# Given the STORY TEXT and the expert SENTIMENT LABEL, return ONLY a JSON object with keys:
# - "story_summary": 1–2 sentences, ≤40 words.
# - "brand_focus": one of ["feature","moderate","passing mention"].
# - "sentiment_label": one of [{labels_csv}].
# - "key_sentiment_driver": 1–2 sentences tied specifically to how the coverage portrays {nes}.
#
# STORY TEXT:
# {text}
#
...
#     try:
#         resp = client.chat.completions.create(
#             model=EX_SUMMARIZER_MODEL,
#             messages=[
#                 {"role": "system", "content": "You produce compact, machine-readable JSON as requested."},
#                 {"role": "user", "content": prompt},
#             ],
#         )
#         mig.add_api_usage(resp, EX_SUMMARIZER_MODEL)  # <-- cost tracking
#         raw = resp.choices[0].message.content or ""
#         # Pull first JSON object (best-effort)
...
#             "brand_focus": str(data.get("brand_focus", "")).strip().lower(),
#             "sentiment_label": str(data.get("sentiment_label", "")).strip().upper(),
#             "key_sentiment_driver": str(data.get("key_sentiment_driver", "")).strip(),
#             "group_id": seed.get("group_id"),
#         }
#         if ex["brand_focus"] not in {"feature", "moderate", "passing mention"}:
#             ex["brand_focus"] = "moderate"
#         if ex["sentiment_label"] not in {s.upper() for s in label_set}:
#             ex["sentiment_label"] = seed.get("label", "NEUTRAL")
#         return ex
#     except Exception:
#         return None
#
# def build_calibration_examples(max_examples: int = EXAMPLE_CAP):
#     """
#     Build/refresh distilled exemplars from the current calibration pool.
#     Saves list of exemplars in st.session_state.calibration_examples.
#     """
#     seeds = list(st.session_state.get("calibration_pool", []))
#     if not seeds:
#         st.session_state.calibration_last_status = "No human-toned seeds available yet."
...
#     errors = 0
#
#     # Summarize concurrently (cap total to avoid big bursts)
#     seeds_to_process = seeds[:50]
#     with ThreadPoolExecutor(max_workers=8) as ex:
#         futs = [ex.submit(_summarize_seed_to_exemplar, s, named_entity, label_set) for s in seeds_to_process]
...
#                 errors += 1
#
#     # Keep it compact and balanced: sample up to max_examples
#     by_label = {}
#     for e in examples:
#         by_label.setdefault(e["sentiment_label"], []).append(e)
#
#     target = []
#     per_label_target = max(1, max_examples // max(1, len(by_label)))
#     for _, items in by_label.items():
#         target.extend(items[:per_label_target])
#
#     if len(target) < max_examples:
#         remaining = [e for e in examples if e not in target]
#         target.extend(remaining[: (max_examples - len(target))])
#
#     st.session_state.calibration_examples = target[:max_examples]
#     st.session_state.calibration_last_status = (
#         f"Built {len(st.session_state.calibration_examples)} examples (from {len(seeds)} seeds, {errors} errors)."
#     )
#
# # ===================== Layout =====================
# col1, col2 = st.columns([5, 2], gap="large")
...
# highlighted_body = highlight_with_tolerant_regex(body_display, keywords)
#
# # ===== Sidebar: Translate + Calibration controls =====
# with sidebar:
#     if st.button("Translate"):
#         try:
#             th = translate(head_raw) if (head_raw or "").strip() else None
...
#
#     st.divider()
#     st.subheader("Calibration (Option B)")
#     st.caption("Build compact examples (summary, focus, driver) from your human labels.")
#
#     # Back-fill seeds from any rows you've already labeled (useful the first time)
#     if st.button("Capture seeds from existing labels"):
#         labeled = st.session_state.df_traditional.loc[
#             st.session_state.df_traditional["Assigned Sentiment"].notna(),
#             ["Group ID", "Headline", "Snippet", "Assigned Sentiment"]
#         ].drop_duplicates("Group ID")
#         added = 0
#         for _, r in labeled.iterrows():
#             gid = int(r["Group ID"])
#             if gid not in st.session_state.calibration_seen_gids:
#                 _add_calibration_seed(
#                     gid,
#                     str(r["Assigned Sentiment"]),
#                     str(r.get("Headline", "") or ""),
#                     str(r.get("Snippet", "") or ""),
#                 )
#                 added += 1
#         st.success(f"Captured {added} new seed(s). Total seeds: {len(st.session_state.calibration_pool)}")
#
#     c1, c2 = st.columns(2)
#     with c1:
#         if st.button("Build/Refresh Examples"):
#             with st.spinner("Building distilled examples…"):
#                 build_calibration_examples(max_examples=EXAMPLE_CAP)
#             st.success(st.session_state.calibration_last_status)
#     with c2:
#         if st.button("Clear Examples"):
#             st.session_state.calibration_examples = []
#             st.session_state.calibration_last_status = "Cleared examples."
#             st.success("Cleared examples.")
#
#     st.caption(f"Seeds captured: {len(st.session_state.calibration_pool)}")
#     if st.session_state.calibration_examples:
#         st.caption(f"Examples ready: {len(st.session_state.calibration_examples)}/{EXAMPLE_CAP}")
#         with st.expander("Preview first 3 examples", expanded=False):
#             for ex in st.session_state.calibration_examples[:3]:
#                 st.markdown(f"**Label:** {ex['sentiment_label']} · **Focus:** {ex['brand_focus']}")
#                 st.markdown(f"_Summary_: {ex['story_summary']}")
#                 st.markdown(f"_Driver_: {ex['key_sentiment_driver']}")
#                 st.markdown("---")
#
# # ===== Left column: Story =====
# with col1:
...
#     accept_help = None if not accept_disabled else "AI label not ready yet."
#
#     # Helper to add a seed when a human label is applied
#     def _record_seed(human_label: str):
#         _add_calibration_seed(
#             current_group_id,
#             human_label,
#             head_raw=head_raw,
...
#                 mask_trad = st.session_state.df_traditional["Group ID"] == current_group_id
#                 st.session_state.df_traditional.loc[mask_trad, "Assigned Sentiment"] = ai_label
#                 # record seed with the **human** label (accepted AI)
#                 _record_seed(ai_label)
#                 st.session_state.counter = min(
#                     len(st.session_state.filtered_stories) - 1,
#                     st.session_state.counter + 1
#                 )
...
#         mask_trad = st.session_state.df_traditional["Group ID"] == current_group_id
#         st.session_state.df_traditional.loc[mask_trad, "Assigned Sentiment"] = clicked_override
#         # record seed with the **human** label (manual override)
#         _record_seed(clicked_override)
#         st.session_state.counter = min(len(st.session_state.filtered_stories) - 1, st.session_state.counter + 1)
#         st.rerun()
#
#     st.divider()
...
# # -------- Calibration helpers (Option B: distilled exemplars) --------
# def _add_calibration_seed(group_id: int, human_label: str, head_raw: str, body_raw: str):
#     """Add a unique seed for this group to the calibration pool."""
#     seen = st.session_state.calibration_seen_gids
#     if group_id in seen:
#         return
#     # Prefer translated text if present
...
#         "label": str(human_label).strip().upper(),
#         "headline": str(story_head or "")[:400],
#         "snippet": str(story_body or "")[:2500],  # keep tidy; summarizer doesn't need full length
#     }
#     st.session_state.calibration_pool.append(seed)
#     seen.add(group_id)
#
# def _summarize_seed_to_exemplar(seed: dict, named_entity: str, label_set: list[str]) -> dict | None:
#     """
#     Turn one seed into a compact exemplar:
#       {story_summary, brand_focus, sentiment_label, key_sentiment_driver}
...
#     return ["POSITIVE","NEUTRAL","NEGATIVE","NOT RELEVANT"]
#
# def build_calibration_examples(max_examples: int = 20):
#     """
#     Build/refresh distilled exemplars from the current calibration pool.
#     Saves list of exemplars in st.session_state.calibration_examples.
#     """
...
#
#     # Keep it compact and balanced: sample up to max_examples
#     # Simple balancing by label then fill remainder
#     by_label = {}
#     for e in examples:
#         by_label.setdefault(e["sentiment_label"], []).append(e)
#
...
#
#     st.session_state.calibration_examples = target[:max_examples]
#     st.session_state.calibration_last_status = f"Built {len(st.session_state.calibration_examples)} examples (from {len(seeds)} seeds, {errors} errors)."
#
# # ===================== Layout =====================
# col1, col2 = st.columns([5, 2], gap="large")
#
...
#         if st.button("Build/Refresh Examples"):
#             with st.spinner("Building distilled examples…"):
#                 build_calibration_examples(max_examples=20)
#             st.success(st.session_state.calibration_last_status)
#     with c2:
#         if st.button("Clear Examples"):
#             st.session_state.calibration_examples = []
...
#     st.caption(f"Seeds captured: {len(st.session_state.calibration_pool)}")
#     if st.session_state.calibration_examples:
#         st.caption(f"Examples ready: {len(st.session_state.calibration_examples)}")
#         with st.expander("Preview first 3 examples", expanded=False):
#             for ex in st.session_state.calibration_examples[:3]:
#                 st.markdown(f"**Label:** {ex['sentiment_label']} · **Focus:** {ex['brand_focus']}")
#                 st.markdown(f"_Summary_: {ex['story_summary']}")
